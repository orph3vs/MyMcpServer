...
class RequestPipeline:
    ...
    def _compose_answer(
        ...
        if law_name and article.get("found") and article_text and article_no:
            ...
            if version_fields.get("시행일자"):
                lines.extend(
                    [
                        "",
                        f"기준 정보: 현재 확인된 시행일자는 {version_fields['시행일자']}입니다.",
                    ]
                )
            ...
            if risk_level == "HIGH":
                lines.extend(
                    [
                        "",
                        "이 질문은 해석상 주의가 필요한 편이라, 실제 적용 시에는 관련 조문과 사실관계를 추가로 확인하는 것이 안전합니다.",
                    ]
                )
        ...
        # Final fallback keeps the prompt validation path meaningful even before a full LLM integration.
        if prompt_payload.get("system") and fallback_answer.strip():
